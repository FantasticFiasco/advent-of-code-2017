...
namespace AdventOfCode.Day2
{
    public class PuzzleShould
    {
        [Theory]
        [InlineData("5 1 9 5\n7 5 3\n2 4 6 8", 18)]
        [InlineData(Input, 50376)]
        public void SolvePart1(string sequence, int expected)
        {
            // Arrange
            var input = ToNumberArray(sequence);

            // Act
            var output = Puzzle.SolvePart1(input);
        ...
        public void SolvePart2(string sequence, int expected)
        {
            ...
            // Act
            var output = Puzzle.SolvePart2(input);

            // Assert
            output.ShouldBe(expected);
        }

        private static int[][] ToNumberArray(string input)
        {
            return input
                .Split('\n')
                .Select(
