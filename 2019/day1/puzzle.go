...
func SolvePart1(masses []int) int {
	...
	for _, mass := range masses {
		fuel += calculateRequiredAmountOfFuel(mass)
	}
	return fuel
}
...
func SolvePart2(masses []int) int {
	fuel := 0
	for _, mass := range masses {
		moduleFuel := []int{calculateRequiredAmountOfFuel(mass)}
		for {
			moduleFuel = append(moduleFuel, max(0, calculateRequiredAmountOfFuel(last(moduleFuel))))
			if last(moduleFuel) == 0 {
				break
			}
		}
		fuel += sum(moduleFuel)
