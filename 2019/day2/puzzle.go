...
func SolvePart1(numbers []int) []int {
	for i := 0; numbers[i] != exit; i += 4 {
		opcode := numbers[i]
		inputIndexX := numbers[i+1]
		inputIndexY := numbers[i+2]
		outputIndex := numbers[i+3]
		switch opcode {
		case add:
			numbers[outputIndex] = numbers[inputIndexX] + numbers[inputIndexY]
		case multiply:
			numbers[outputIndex] = numbers[inputIndexX] * numbers[inputIndexY]
		default:
...
func SolvePart2(memory []int, searchedOutput int) (int, int) {
	for noun := 0; noun < 100; noun++ {
		for verb := 0; verb < 100; verb++ {
			clonedMemory := append([]int(nil), memory...)
			clonedMemory[1] = noun
			clonedMemory[2] = verb
			output := first(SolvePart1(clonedMemory))
			if output == searchedOutput {
				return noun, verb
			}
		}
	}
	panic("Searched output not found in memory")
}
