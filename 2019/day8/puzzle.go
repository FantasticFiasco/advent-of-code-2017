...
func SolvePart1(width, height int, pixels []int) int {
	pixelsPerLayer := width * height
	nbrOfLayers := len(pixels) / pixelsPerLayer
	var layers []layerStats
	for layerIndex := 0; layerIndex < nbrOfLayers; layerIndex++ {
		layer := layerStats{index: layerIndex}
		for i := 0; i < pixelsPerLayer; i++ {
			switch pixels[layerIndex*pixelsPerLayer+i] {
			case 0:
				layer.nbrOfZeroes++
			case 1:
				layer.nbrOfOnes++
	...
	layerWithLeastZeroes := findLayerWithLeastZeroes(layers)
	return layerWithLeastZeroes.nbrOfOnes * layerWithLeastZeroes.nbrOfTwos
}

/*
Now you're ready to decode the image. The image is rendered by stacking the layers and aligning the pixels with the same
positions in each layer. The digits indicate the color of the corresponding pixel: 0 is black, 1 is white, and 2 is
transparent.
...
func SolvePart2(width, height int, pixels []int) []int {
	pixelsPerLayer := width * height
	nbrOfLayers := len(pixels) / pixelsPerLayer
	var layers [][]int
	for layerIndex := 0; layerIndex < nbrOfLayers; layerIndex++ {
		layers = append(layers, pixels[layerIndex*pixelsPerLayer:layerIndex*pixelsPerLayer+pixelsPerLayer])
	}
	return render(width, height, layers)
}
...
func findLayerWithLeastZeroes(layerStats []layerStats) layerStats {
	layerWithLeastZeroes := layerStats[0]
	for i := 1; i < len(layerStats); i++ {
		if layerStats[i].nbrOfZeroes < layerWithLeastZeroes.nbrOfZeroes {
			layerWithLeastZeroes = layerStats[i]
		}
	}
	return layerWithLeastZeroes
}

func render(width, height int, layers [][]int) []int {
	var screen []int
	nbrOfLayers := len(layers)
	for i := 0; i < width*height; i++ {
	layerIteration:
		for layerIndex := 0; layerIndex < nbrOfLayers; layerIndex++ {
			switch layers[layerIndex][i] {
			case 0:
